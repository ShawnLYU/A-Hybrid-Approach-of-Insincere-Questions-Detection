...
We conducted three different experiments to test the effects of hyperparameters on the two models to get a better comprehension of how they differ. In general, models with statistical features performed better than baseline models. Since we restricted our sample sizes and numbers of epochs to accommodate for the limited computational resources, we expected a further expansion on model complexity to result in better performance. In addition, the labels of our data were imbalanced and contained noises, therefore the prediction results might not serve as ideal measurements of our models. In the following sections, we will also examine validation losses to compare models in different experiments. 


\subsubsection{Effect of Dropout} % (fold)
\label{ssub:effect_of_dropout}

Dropout layers have been shown to decrease model performance on sarcasm detection \citep{ghosh2016}. In our experiment, the baseline model showed a similar pattern where the presence of the dropout layer resulted in lower accuracy. This observation could indicate that the removal of dropout layers were beneficial for this task, but the reason was also possibly that the model was underfitting whereas dropout was a mechanism to prevent overfitting. Interestingly, the models with statistical features were not affected by the dropout layer in terms of training and testing accuracy. 
...
According to figure \ref{figure:dropout}, models without dropout layers reached lower validation loss but were also less stable. 
